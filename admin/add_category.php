...
if (isset($_POST["submit"])) {
    ...
    $order            = trim($_POST["order"]);
        ...
        if ($addflag == 1) {
            mysql_query("INSERT INTO `category`(`name`, `order`)
                                          VALUES('$name', '$order')") or die(mysql_error());
            insmess();
             
            header("refresh: 1; add_category.php?mode=true");
    }else{
          $mvouch = $qvouch;
            mysql_query("UPDATE `category` SET `name`='$name', `order`='$order'
                                                WHERE code = '$mvouch' ") or die(mysql_error());
            updmess();
            header("refresh: 1; sort_cat.php?vouchno=$mvouch");
        }
...
if ($addflag == 0) {
    ...
    $result = mysql_query("SELECT *  FROM `category` WHERE code='$getvouch'") or die(mysql_error());
                  ...
                  <div id="div-4" class="accordion-body collapse in body">
                    <form class="form-horizontal" action='' method='POST'>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-lg-5">
                          <div class="input-group">
                              <input type="text" maxlength='2'  id="invno" name="order"value="<?php echo ($addflag == 0) ? $get['order'] : ''; ?>"   class="form-control" />    
                          </div>
                        </div>
                      </div>
                      <div class="form-group">
                        ...
                        <div class="col-lg-5">
                          <div class="input-group">
                           ...
                           <input type='button' name='exit' class='btn btn-danger' value='Exit' onClick="window.location.href='sort_cat.php'">
                          </div>
                        </div>
                      </div>
                    </form>
